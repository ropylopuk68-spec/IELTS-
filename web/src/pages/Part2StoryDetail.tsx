...
export function Part2StoryDetail() {
  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      ...
      {/* Recording Panel */}
      <div className="fixed bottom-6 right-6 bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-purple-100/50 w-[380px]">
        <div className="px-5 py-4">
          <div className="flex items-center justify之间 mb-3">
            <div className="flex items-center gap-2">
              <Mic className="w-5 h-5 text-purple-600" />
              <span className="font-bold text-gray-800">自我录音</span>
            </div>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className={`px-2 py-1 rounded-full text-xs font-bold ${isRecording ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>● {isRecording ? 'Recording' : 'Idle'}</div>
          </div>

          {/* Controls */}
          <div className="flex items-center justify-center gap-3">
            <button
              onClick={() => { setIsTimerRunning(true); startRecording(); }}
      ...
      {uploadedAudioUrl && (
        <div className="fixed bottom-6 left-6 bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-purple-100/50 w-[380px]">
          <div className="px-5 py-4">
            <div className="flex items-center gap-2 mb-3">
              <Upload className="w-5 h-5 text-purple-600" />
              <span className="font-bold text-gray-800">上传音频</span>
            </div>
            <audio ref={uploadedAudioRef} src={uploadedAudioUrl} controls className="w-full" />
            <div className="flex items-center justify-center gap-3 mt-3">
              <button 
                onClick={playUploadedAudio}
                className="px-3 py-2 rounded-lg bg-indigo-500 text-white hover:bg-indigo-600 text-sm"
                disabled={isPlayingUploadedAudio}
              >
                播放
              </button>
              <button 
                onClick={pauseUploadedAudio}
                className="px-3 py-2 rounded-lg bg-gray-500 text-white hover:bg-gray-600 text-sm"
                disabled={!isPlayingUploadedAudio}
              >
                暂停
              </button>
