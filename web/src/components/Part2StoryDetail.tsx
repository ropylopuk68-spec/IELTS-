      ...
      <div className="max-w-5xl mx-auto pb-32">
        ...
        <div className="bg-gradient-to-br from-purple-50/30 to-pink-50/30 backdrop-blur-xl rounded-2xl shadow-xl border border-purple-100/50 mb-6">
          <button
            ...
            className="w-full flex items-center justify之间 p-6 hover:bg紫-50/30 transition-colors"
          >
          ...
          {showAnswers && (
            <div className="px-6 pb-6">
                  ...
                  <div
                    ...
                    className={`p-5 rounded-lg transition-all duration-300 ${
                      currentParagraphIndex === index
                        ? 'bg紫-100/80 shadow-md scale-[1.01]'
                        : 'bg白/60 hover:bg白/80'
                    }`}
                  >
                    <p
              ...
              {/* Control Bar */}
              <div className="pt-4 border-t border紫-200/50">
                <div className="flex items-center justify中心 gap-6">
                  <button
                    onClick={playFullAnswer}
                    className="p-4 rounded-full bg-gradient-to-r from紫-500 to-pink-500 text白 hover:shadow-lg transition-all"
                    ...
                    {[0.5, 0.75, 1, 1.25, 1.5].map((speed) => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg text-sm font-medium transition-colors ${
                          playbackSpeed === speed
                            ? 'bg紫-500 text白'
                            : 'bg灰-100 text-gray-700 hover:bg灰-200'
                        }`}
                      >
                        {speed}x
                      </button>
        ...
        {/* Voice Follow Module */}
        <div className="bg-gradient-to-br from绿-50/30 to-emerald-50/30 backdrop-blur-xl rounded-2xl shadow-xl border border绿-100/50 mb-6">
          <button
            onClick={() => {
              playSound(600, 100);
              setShowVoiceFollow(!showVoiceFollow);
            }}
            className="w-full flex items-center justify之间 p-6 hover:bg绿-50/30 transition-colors"
          >
          ...
          {showVoiceFollow && (
            <div className="px-6 pb-6">
              <div className="flex items-center gap-4 mb-4">
                <label htmlFor="audio-upload" className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from绿-500 to-emerald-500 text白 rounded-xl hover:shadow-lg transition-all cursor-pointer">
              ...
              {uploadedAudioUrl && (
                <div className="mt-4">
                  ...
                  <div className="mt-2 flex items-center justify中心">
                    <button
                      onClick={isPlayingUploadedAudio ? pauseUploadedAudio : playUploadedAudio}
                      className="p-4 rounded-full bg-gradient-to-r from绿-500 to-emerald-500 text白 hover:shadow-lg transition-all"
        ...
        {currentTopic.analysis && (
          <div className="bg-gradient-to-br from黄-50/30 to橙-50/30 backdrop-blur-xl rounded-2xl shadow-xl border border黄-100/50 mb-6">
            <button
              onClick={() => {
                playSound(700, 100);
                setShowAnalysis(!showAnalysis);
              }}
              className="w-full flex items-center justify之间 p-6 hover:bg黄-50/30 transition-colors"
            >
        ...
        {currentTopic.keywordsMemo && currentTopic.keywordsMemo.length > 0 && (
          <div className="bg-gradient-to-br from粉-50/30 to-rose-50/30 backdrop-blur-xl rounded-2xl shadow-xl border border粉-100/50 mb-6">
            <button
              onClick={() => {
                playSound(800, 100);
                setShowKeywords(!showKeywords);
              }}
              className="w-full flex items-center justify之间 p-6 hover:bg粉-50/30 transition-colors"
            >
              ...
              <div className="px-6 pb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {currentTopic.keywordsMemo.map((keyword, index) => (
                    <div key={index} className="flex items-center gap-3 p-3 bg白/60 rounded-lg">
        ...
        <div className="bg-gradient-to-br from靛-50/30 to紫-50/30 backdrop-blur-xl rounded-2xl shadow-xl border border靛-100/50 mb-6">
          <button
            onClick={() => setShowRecording(!showRecording)}
            className="w-full flex items-center justify之间 p-6 hover:bg靛-50/30 transition-colors"
          >
          ...
          {showRecording && (
            <div className="px-6 pb-6 relative">
              ...
              <div className="absolute top-0 right-6 flex items-center gap-2 text-2xl font-bold text靛-600">
                <span>⏰</span>
              ...
              <div className="flex justify中心 pt-8">
                <button
                  onClick={() => {
                    if (!isTimerRunning) {
                      startRecording();
                  ...
                  className={`px-8 py-4 rounded-2xl font-bold text-lg transition-all shadow-lg ${
                    isTimerRunning
                      ? 'bg-red-500 text白 scale-105 shadow-red-300'
                      : 'bg-gradient-to-r from靛-500 to紫-500 text白 hover:shadow-xl'
                  }`}
                >
