...
export function Part2TopicList() {
  ...
  const getStoryGroupBg = (groupName: string, isHovered: boolean = false) => {
    ...
    if (groupName.includes('城市')) return isHovered ? 'bg-purple-100/90' : 'bg-purple-50/80';
    ...
    if (groupName.includes('电影')) return isHovered ? 'bg-purple-100/90' : 'bg-purple-50/80';
    ...
    if (groupName.includes('看书')) return isHovered ? 'bg-purple-100/90' : 'bg-purple-50/80';
    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-blue-50/40 via-purple-50/40 to-pink-50/40">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-6">
          <h1 
            className="text-3xl font-bold bg-gradient-to-r from-purple-500 via-purple-400 to-pink-400 bg-clip-text text-transparent inline-block"
          >
            {categoryInfo.name}
          </h1>
        </div>
                                  ...
                                  <td className="py-3 text-center bg-white/40 backdrop-blur-sm transition-all duration-300" style={{ width: '20%' }} onClick={(e) => e.stopPropagation()}>
                                    {examTakers[topic.id] > 0 ? (
                                      <input
                                        ...
                                        className="w-20 px-3 py-1.5 text-center border border-purple-300/50 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-200 text-base font-bold text-purple-700 bg-white/80 backdrop-blur-sm shadow-sm transition-all"
                                        min="0"
                                      />
                                    ) : (
